...
def build_pdf(json_path, output_path):
    ...
    story.append(Paragraph("跨学科长尾科研知识多目标进化报告  ·  7目标可行性强化版",
    ...
    insights = [
        ("算法改进效果", "可行/合理大幅提升",
         f"5项算法改进使初始种群可行性从 v2 的 4.0→{init_stats['avgs'][5]:.1f}（+{init_stats['avgs'][5]-4.0:.1f}），"
         f"合理性从 5.4→{init_stats['avgs'][6]:.1f}（+{init_stats['avgs'][6]-5.4:.1f}）。"
         "偏置权重（f6/f7 α=3x）+ 初始种群提示偏置是最关键的改进。"),
        ("惩罚约束效果", "防止低分解蔓延",
         f"ε-约束惩罚（阈值=5.5，惩罚权重=3.0）确保了整个演化过程中"
         f"可行性均值维持在 {min(r['avgs'][5] for r in gen_stats):.1f}~{max(r['avgs'][5] for r in gen_stats):.1f}、"
         f"合理性在 {min(r['avgs'][6] for r in gen_stats):.1f}~{max(r['avgs'][6] for r in gen_stats):.1f} 区间，"
         "有效防止了低可行/低合理解在邻居替换中扩散。"),
        ("研究知识质量对比", "v2 vs v3",
         "v2 种群以抽象量子-天体理论为主（可行=4, 合理=5），研究路径不清晰；"
         "v3 种群转向具体实验方向：古DNA表观遗传、冰川微生物基因组、工业废水定向进化等，"
         "每个条目都有明确的现有技术支撑，科学假设逻辑严密可证伪。"),
        ("涌现的研究范式", "生物-环境交叉为主",
         "v3 的 Pareto 前沿以「表观遗传学×极端环境生态学×材料科学」为核心，"
         "相比 v2 的宇宙量子假说更加落地，体现了强化可行性/合理性约束后"
         "演化方向自然向「高质量可研究科学问题」收敛的算法有效性。"),
    ]
    ins_data = [["维度", "洞察", "分析"]]
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        f"MOEA/D × Claude 自动演化系统（5项算法改进）  |  模型: claude-sonnet-4-5  |  "
...
if __name__ == "__main__":
    build_pdf(
        os.path.expanduser("~/moead_science_v3_results.json"),
        os.path.expanduser("~/moead_science_v3_report.pdf")
    )
